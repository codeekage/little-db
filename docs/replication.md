...
The encoded record is the *exact* byte sequence the leader appended to its
...
- **Raft**: ~1500–2500 LOC of correctness-critical code, plus partition
  ...
  *correctly under -race and partition tests* is days of work, not hours.
...
- **Multi-Paxos**: harder than Raft, not easier. The original Paxos paper
  ...
  designed to be more *understandable* than Paxos for this exact reason
...
| Match to append-only segments | Excellent — segments *are* the log   | Same, but the leader-instance ambiguity adds work |
...
| `internal/replication/raft.go` *(new)* | State machine: Follower / Candidate / Leader. Election timer. AppendEntries / RequestVote RPCs over the existing wire framing.       |
| `internal/replication/log.go` *(new)* | Persistent Raft log (term, index, command). Reuses the segment-and-CRC infrastructure for storage.                                   |
...
*state machine* applied to by the Raft log, or whether Raft is layered
*above* an unmodified Engine and writes are applied as a side effect of
...
- Ongaro, D. (2014). *Consensus: Bridging Theory and Practice.* Stanford
  PhD thesis. The canonical reference, including log compaction and
  membership changes.
- Lamport, L. (2001). *Paxos Made Simple.* SIGACT News 32(4).
- Howard, H., & Mortier, R. (2020). *Paxos vs Raft: Have We Reached
  Consensus on Distributed Consensus?* PaPoC.
- Moraru, I., Andersen, D. G., & Kaminsky, M. (2013). *There Is More
  Consensus in Egalitarian Parliaments.* SOSP. (EPaxos.)
- Sheehy, J., & Smith, D. (2010). *Bitcask: A Log-Structured Hash Table
  for Fast Key/Value Data.* The Bitcask paper, for the storage layer
  this replication design sits on.
- Kingsbury, K. *Jepsen* (`jepsen.io/analyses`). The empirical record on
