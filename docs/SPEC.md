...
**Trade — hint fast-path skips per-record data CRC verification at Open.**
The hint file's own CRC32C covers its keys + offsets + lengths, but it
carries no digest of the value bytes it points into. So a hinted segment
whose data file was silently corrupted (bit-rot, partial fsync of a body
write that the kernel later didn't reconcile, etc.) will pass `Open` even
though a record in it would fail CRC under a full data scan. The
mid-segment-CRC guarantee from §9 is therefore weaker in the presence of
hints: it applies to **the active segment** (always data-scanned) and to
**any non-active segment whose hint file is missing or unparseable**
(scan fallback). Mid-segment corruption inside a successfully-hinted
segment surfaces lazily — currently on a `Get` that would have to read
the corrupted bytes, the engine returns the bytes as-is without
re-verifying CRC, so detection is deferred to whatever external integrity
check the caller runs. Strengthening hints with a per-entry value digest
(e.g. carrying each record's original CRC32C in the hint entry, then
re-checking it on the first `Get` against that entry or eagerly at
`Open`) is the obvious next step and listed under §11 future work; it
trades hint-file size (+4 B per entry) for restoring the full §9
...
| Single segment file corruption                              | A trailing torn write is truncated away on recovery (its writes were never acked). A CRC mismatch on a record that fits entirely within the segment file aborts `Open` rather than silently dropping data — the operator decides whether to repair, restore, or run with `AllowCorruption` (future). Caveat: this guarantee applies to scanned segments; hint-replayed segments (see §7.4) skip the per-record data CRC at `Open` and surface mid-segment corruption only on a later `Get`.                                                                                                                                                                                                                                  |
...
| Host crash after manifest rename but before directory fsync | The new MANIFEST is in the directory entry but its visibility is not guaranteed to survive a crash. The engine reports `ErrManifestPublishedButUncertain` from `writeManifest`. During rotation, the engine then enters a fatal write-disabled state (`ErrWritesDisabled` on subsequent `Put` / `Delete` / `BatchPut`) and preserves the new segment file on disk without installing it as the active segment. Reads continue to serve from prior sealed segments. The operator restarts; `Open`'s `sweepOrphans` reconciles whichever side of the rename survived. During compaction the in-memory view advances to the post-compaction state but both old and new segment files are preserved on disk for the same reason. |
